...
def _init_test_db(db_path: Path, initial_cash: float = 1000.0) -> None:
    ...
    conn.execute(
        ...
        "VALUES (?, ?, 0.0, ?, ?)",
        (initial_cash, initial_cash, initial_cash, datetime.datetime.now()),
    )
    conn.commit()
    conn.close()
...
def test_paper_session_executes_signal_and_reconciles(tmp_path, mock_circuit_breaker):
    ...
    assert pf[0] == 1000.0  # saldo_disponivel (capital entregue ao bot) permanece 1000.0
    assert pf[1] > 0.0      # capital alocado em posições ativas
    saldo_livre = pf[0] - pf[1]
    assert saldo_livre < 1000.0  # caixa livre reduziu exatamente pela alocação
    assert pytest.approx(saldo_livre + pf[1]) == 1000.0
    conn.close()

    # Validação do Journal em disco
    ...
    # Validação do relatório JSON e manifesto
...
def test_paper_session_idempotent_on_replay_no_duplicate_orders(tmp_path, mock_circuit_breaker):
    ...
    # 2ª Execução (Replay da mesma sessão ou nova sessão com a mesma posição aberta)
...
def test_paper_session_resumes_interrupted_session(tmp_path, mock_circuit_breaker):
    ...
    sig_petr = {
        "ticker": "PETR4", "signal": "BUY", "last_price": 30.0,
        "target_price": 33.0, "stop_loss": 28.5, "confidence": 80, "reason": "Sinal 1"
    }
    sig_vale = {
        "ticker": "VALE3", "signal": "BUY", "last_price": 60.0,
        "target_price": 66.0, "stop_loss": 57.0, "confidence": 80, "reason": "Sinal 2"
    }

    # 1. Ciclo com apenas o primeiro ativo processado
    ...
    # 2. Retomada da mesma sessão com ambos os ativos
    ...
    # PETR4 foi pulado (já executado), VALE3 foi executado
...
def test_paper_session_exit_management_and_reconciliation(tmp_path, mock_circuit_breaker):
    ...
    # 1. Abre posição em PETR4 @ 30.0 (alvo 33.0)
    sig = {
        "ticker": "PETR4", "signal": "BUY", "last_price": 30.0,
        "target_price": 33.0, "stop_loss": 28.5, "confidence": 80, "reason": "Entrada"
    }
    rep1 = runner.run_cycle(signals=[sig])
    assert rep1.orders_executed == 1

    # 2. Próximo ciclo com preço de mercado acima do alvo (34.0)
    ...
    pf = conn.execute("SELECT saldo_disponivel, em_posicoes FROM portfolio ORDER BY id DESC LIMIT 1").fetchone()
    assert pf[1] == 0.0
    assert pf[0] > 1000.0  # Saldo cresceu pelo lucro do trade
    conn.close()
...
def test_paper_session_risk_rejection_fail_closed(tmp_path, mock_circuit_breaker):
    ...
    # Sinal HOLD deve ser rejeitado pelo RiskManager
    signals = [
        {
            "ticker": "PETR4",
            "signal": "HOLD",
...
def test_paper_session_guarantees_zero_real_broker_calls(tmp_path, mock_circuit_breaker):
    ...
    signals = [
        {
            "ticker": "PETR4", "signal": "BUY", "last_price": 30.0,
            "target_price": 33.0, "stop_loss": 28.5, "confidence": 80, "reason": "Teste"
        }
    ]

    # Patch defensivo em urllib e sockets para interceptar qualquer vazamento de rede
