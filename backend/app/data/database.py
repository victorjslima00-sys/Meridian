...
def init_db():
    ...
    # ------------------------------------------------------------------
    # patrimonio_total : capital do usuário que a IA NUNCA toca
    # saldo_disponivel : teto máximo que a IA pode usar (usuário define)
    # em_posicoes      : soma das alocações nas ordens abertas
    #
    # Invariante: em_posicoes <= saldo_disponivel (sempre)
    # Saldo livre (calculado) = saldo_disponivel - em_posicoes
    # ------------------------------------------------------------------
    cursor.execute('''
    ...
    existing = {row[1] for row in cursor.execute("PRAGMA table_info(portfolio)").fetchall()}
    if "initial_capital" in existing and "patrimonio_total" not in existing:
        cursor.execute("ALTER TABLE portfolio ADD COLUMN patrimonio_total REAL DEFAULT 0.0")
        cursor.execute("ALTER TABLE portfolio ADD COLUMN saldo_disponivel REAL DEFAULT 100.0")
        cursor.execute("ALTER TABLE portfolio ADD COLUMN em_posicoes REAL DEFAULT 0.0")
        cursor.execute("""
...
def get_portfolio() -> Dict[str, Any]:
    ...
    if not row:
        return {"patrimonio_total": 0.0, "saldo_disponivel": 100.0,
                "em_posicoes": 0.0, "saldo_livre": 100.0}
...
def depositar_no_disponivel(valor: float) -> Dict[str, Any]:
    ...
    if valor > patrimonio:
        conn.close()
        return {"ok": False, "error": f"Patrimônio insuficiente (total: R$ {patrimonio:.2f})."}
    ...
    conn.commit()
    conn.close()
    return {"ok": True, "patrimonio_total": patrimonio - valor, "saldo_disponivel": disponivel + valor}
...
def retirar_do_disponivel(valor: float) -> Dict[str, Any]:
    ...
    if valor > livre:
        conn.close()
        return {"ok": False, "error": f"Saldo livre insuficiente (livre: R$ {livre:.2f})."}
